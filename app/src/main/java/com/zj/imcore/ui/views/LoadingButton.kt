...
class LoadingButton @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : View(context, attrs, defStyle) {
    ...
    init {
        if (attrs != null) {
            ...
            mColorPrimary = ta.getColor(R.styleable.LoadingButton_lb_btnColor, Color.BLUE)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(measureDimension((DEFAULT_WIDTH * mDensity).toInt(), widthMeasureSpec), measureDimension((DEFAULT_HEIGHT * mDensity).toInt(), heightMeasureSpec))
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                val canClick = (mRawX in left..right) && (mRawY in top..bottom)
                mTouchX = 0f
                mTouchY = 0f
                mRawX = 0f
                mRawY = 0f
            ...
            STATE_BUTTON, STATE_ANIMATION_STEP1 -> {
                ...
                if (mCurrentState == STATE_BUTTON) {
                    ...
                    if ((mTouchX > 0 || mTouchY > 0) && rippleEnable) {
                        canvas.drawRoundRect(0f, mPadding, width.toFloat(), viewHeight - mPadding, mButtonCorner, mButtonCorner, mRipplePaint)
                    }
                }
            }
    ...
    private fun measureDimension(defaultSize: Int, measureSpec: Int) = when (MeasureSpec.getMode(measureSpec)) {
    ...
    private fun playStartAnimation(isReverse: Boolean) {
        val viewHeight = max(height, mMinHeight.toInt())
        val animator = ValueAnimator.ofInt(if (isReverse) width / 2 - viewHeight / 2 else 0, if (isReverse) 0 else width / 2 - viewHeight / 2).apply {
            duration = 400
            interpolator = AccelerateDecelerateInterpolator()
            startDelay = 100
            addUpdateListener { valueAnimator ->
        ...
        val animator2 = ValueAnimator.ofInt(if (isReverse) mRadius else 0, if (isReverse) 0 else mRadius).apply {
            duration = 240
            interpolator = AccelerateDecelerateInterpolator()
            addUpdateListener { valueAnimator ->
        ...
        val loadingAnimator = ValueAnimator.ofInt(30, 300).apply {
            duration = 1000
    ...
    private fun playFailedAnimation() {
        ...
        val failedAnimator = ValueAnimator.ofFloat(0.0f, 1.0f).apply {
            duration = 300
            interpolator = AccelerateDecelerateInterpolator()
            addUpdateListener { valueAnimator ->
                val value = valueAnimator.animatedValue as Float
        ...
        val failedAnimator2 = ValueAnimator.ofFloat(0.0f, 1.0f).apply {
            duration = 300
            interpolator = AccelerateDecelerateInterpolator()
            addUpdateListener { valueAnimator ->
                val value = valueAnimator.animatedValue as Float
    ...
    private fun cancel() {
        mCurrentState = STATE_STOP_LOADING
        ValueAnimator.ofInt(360 - mAngle, 360).apply {
            duration = 240
            interpolator = DecelerateInterpolator()
            addUpdateListener { valueAnimator ->
                mEndAngle = valueAnimator.animatedValue as Int
...
private fun Paint.setShadowDepth(depth: Float) {
    this.setShadowLayer(depth, 0f, 2f, 0x6F000000)
}