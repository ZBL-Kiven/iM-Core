...
public class BezierRadarHeader extends InternalAbstract implements RefreshHeader {
    ...
    protected void drawDot(Canvas canvas, int width, int height) {
        if (mDotAlpha > 0) {
            ...
            float wide = (width * 1f / num) * mDotFraction - ((mDotFraction > 1) ? ((mDotFraction - 1) * (width * 1.0f / num) / mDotFraction) : 0);//y1 = t*(w/n)-(t>1)*((t-1)*(w/n)/t)
    ...
    @Override
    public void onReleased(@NonNull final RefreshLayoutIn refreshLayoutIn, int height, int maxDragHeight) {
        mWaveTop = height;
        mWavePulling = false;
