package com.zj.list.refresh.views;
...
import com.zj.list.refresh.constance.DimensionStatus;
import com.zj.list.refresh.constance.RefreshState;
import com.zj.list.refresh.constance.SpinnerStyle;
import com.zj.list.refresh.footer.BallPulseFooter;
import com.zj.list.refresh.header.BezierRadarHeader;
import com.zj.list.refresh.impl.RefreshContentWrapper;
import com.zj.list.refresh.impl.RefreshFooterWrapper;
import com.zj.list.refresh.impl.RefreshHeaderWrapper;
import com.zj.list.refresh.interfaces.*;
import com.zj.list.refresh.listeners.*;
import com.zj.list.refresh.util.DelayedRunnable;
import com.zj.list.refresh.util.DensityUtil;
import com.zj.list.refresh.util.ScrollBoundaryUtil;
import com.zj.list.refresh.util.ViscousFluidInterpolator;
...
import static com.zj.list.refresh.util.DensityUtil.dp2px;
import static com.zj.list.refresh.util.SmartUtil.*;
import static java.lang.System.currentTimeMillis;

@SuppressWarnings("unused")
@SuppressLint("RestrictedApi")
public class RefreshLayout extends ViewGroup implements RefreshLayoutIn, NestedScrollingParent, NestedScrollingChild {

    private static final int FINISH_LOAD_MORE = 500;
    private static final int FINISH_REFRESH = 2000;
    ...
    protected boolean mManualLoadMore;//是否手动设置过LoadMore，用于智能开启
    protected boolean mManualNestedScrolling;//是否手动设置过 NestedScrolling，用于智能开启
    protected boolean mManualHeaderTranslationContent;//是否手动设置过内容视图拖动效果
    ...
    public RefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setNestedScrollingEnabled(ta.getBoolean(R.styleable.RefreshLayout_srlEnableNestedScrolling, false));
        ...
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.RefreshLayout_srlHeaderInsetStart, 0);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.RefreshLayout_srlFooterInsetStart, 0);
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                final RefreshLayoutParams lp = (RefreshLayoutParams) headerView.getLayoutParams();
...
//                    mRefreshHeader.onInitialized(mKernel, mHeaderHeight, mHeaderExtendHeight);
            ...
            if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
                ...
                final RefreshLayoutParams lp = (RefreshLayoutParams) footerView.getLayoutParams();
                final int widthSpec = ViewGroup.getChildMeasureSpec(widthMeasureSpec, lp.leftMargin + lp.rightMargin, lp.width);
                int heightSpec = heightMeasureSpec;
            ...
            if (mRefreshContent != null && mRefreshContent.getView() == child) {
                ...
                final RefreshLayoutParams lp = (RefreshLayoutParams) contentView.getLayoutParams();
...
//                    mRefreshContent.onActionUpOrCancel();
    ...
    protected void setStateDirectLoading() {
        if (mState != RefreshState.Loading) {
            ...
            if (mLoadMoreListener != null) {
                ...
                loadMoreCompleted(FINISH_LOAD_MORE);
            }
            if (mRefreshFooter != null) {
        ...
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mRefreshListener != null) {
                    ...
                    refreshCompleted(FINISH_REFRESH);
                }
                if (mRefreshHeader != null) {
    ...
    //<editor-fold desc="布局参数 RefreshLayoutParams">
    @Override
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        return p instanceof RefreshLayoutParams;
    }

    @Override
    protected RefreshLayoutParams generateDefaultLayoutParams() {
        return new RefreshLayoutParams(MATCH_PARENT, MATCH_PARENT);
    }

    @Override
    protected RefreshLayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        return new RefreshLayoutParams(p);
    }

    @Override
    public RefreshLayoutParams generateLayoutParams(AttributeSet attrs) {
        final View thisView = this;
        return new RefreshLayoutParams(thisView.getContext(), attrs);
    }

    public static class RefreshLayoutParams extends MarginLayoutParams {

        RefreshLayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
        ...
        RefreshLayoutParams(int width, int height) {
            super(width, height);
        }

        RefreshLayoutParams(ViewGroup.LayoutParams source) {
            super(source);
        }
...
//            mHeaderExtendHeight = (int) Math.max((mHeaderHeight * (mHeaderMaxDragRate - 1)), 0);
    ...
    public void setRefreshHeader(@NonNull RefreshHeader header) {
        setRefreshHeader(header, MATCH_PARENT, WRAP_CONTENT);
    }
    ...
    public RefreshLayout setRefreshHeader(@NonNull RefreshHeader header, int width, int height) {
        ...
        if (header.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
            super.addView(mRefreshHeader.getView(), 0, new RefreshLayoutParams(width, height));
        } else {
    ...
    public void setRefreshFooter(@NonNull RefreshFooter footer) {
        setRefreshFooter(footer, MATCH_PARENT, WRAP_CONTENT);
    }
    ...
    public RefreshLayout setRefreshFooter(@NonNull RefreshFooter footer, int width, int height) {
        ...
        if (mRefreshFooter.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
            super.addView(mRefreshFooter.getView(), 0, new RefreshLayoutParams(width, height));
        } else {
    ...
    public RefreshLayout setRefreshContent(@NonNull View content, int width, int height) {
        ...
        super.addView(content, 0, new RefreshLayoutParams(width, height));
        if (mRefreshHeader != null && mRefreshHeader.getSpinnerStyle() == SpinnerStyle.FixedBehind) {
     ...
     */
    @Override
    public void setPrimaryColors(@ColorInt int... primaryColors) {
        if (mRefreshHeader != null) {
     ...
     */
    @Override
    public RefreshLayout refreshCompleted() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return refreshCompleted(Math.max(0, 300 - (int) passTime));//保证刷新动画有300毫秒的时间
    }
     ...
     */
    @Override
    public RefreshLayout loadMoreCompleted() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return loadMoreCompleted(Math.max(0, 100 - (int) passTime));//保证加载动画有300毫秒的时间
    }
     ...
     */
    @Override
    public RefreshLayout refreshCompleted(int delayed) {
        return refreshCompleted(delayed, true);
    }
     ...
     */
    @Override
    public RefreshLayout refreshCompleted(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return refreshCompleted(success ? Math.max(0, 300 - (int) passTime) : 0, success);//保证加载动画有300毫秒的时间
    }
     ...
     */
    @Override
    public RefreshLayout refreshCompleted(int delayed, final boolean success) {
                ...
                if (mState == RefreshState.Refreshing && mRefreshHeader != null && mRefreshContent != null) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        if (mIsBeingDragged) {
                            ...
                            RefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mTouchY + mSpinner - mTouchSlop * 2, 0));
                            RefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mTouchY + mSpinner, 0));
                        }
                        if (mSpinner > 0) {
     ...
     */
    @Override
    public RefreshLayout loadMoreCompleted(int delayed) {
        return loadMoreCompleted(delayed, true, false);
    }
     ...
     */
    @Override
    public RefreshLayout loadMoreCompleted(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return loadMoreCompleted(success ? Math.max(0, 300 - (int) passTime) : 0, success, false);
    }
     ...
     */
    @Override
    public RefreshLayout loadMoreCompleted(int delayed, final boolean success, final boolean noMoreData) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                if (mState == RefreshState.Loading && mRefreshFooter != null && mRefreshContent != null) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged) {
                            ...
                            RefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mTouchY + offset + mTouchSlop * 2, 0));
                            RefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mTouchY + offset, 0));
                        } else if (mTotalUnconsumed != 0) {
                            mDragDirection = 'h';
                            mTotalUnconsumed = 0;
                            final long time = System.currentTimeMillis();
                            RefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_CANCEL, mLastTouchX, mTouchY + offset, 0));
                        }
                        //准备：偏移并结束状态
                        postDelayed(new Runnable() {
                            @Override
     ...
     */
    @Override
    public RefreshLayout loadMoreCompletedWithNoMoreData() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
        return loadMoreCompleted(Math.max(0, 300 - (int) passTime), true, true);
    }
