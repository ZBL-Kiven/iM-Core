...
@Suppress("unused")
class Query<IN, T : Any> private constructor(private val cls: Class<T>?, private val onCompleted: ((Query<IN, T>) -> ReceiveListener<IN, T>)? = null) {
    ...
    companion object {
        ...
        fun <IN, T : Any> from(cls: Class<T>, onCompleted: (Query<IN, T>) -> ReceiveListener<IN, T>): QueryData<T> {
            return QueryData(Query(cls, onCompleted))
        }

        fun <T : Any> from(cls: Class<T>): QueryData<T> {
            return QueryData(Query<Any, T>(cls))
        }

        fun allEvents(): Query<*, Any> {
            return Query<Any, Any>(Any::class.java)
        }
    }
    ...
    fun needLessLocal(): Query<IN, T> {
        needLocal = false
        return this
    }

    fun sort(comparator: Comparator<T>): Query<IN, T> {
        this.comparator = comparator
        return this
    }

    fun sort(sortBy: String, sortType: SortType): Query<IN, T> {
        this.sortMode = Sort(sortBy, sortType)
        return this
    }

    fun maxCount(maxCount: Int): Query<IN, T> {
        this.maxCount = max(-1, maxCount)
        return this
    }

    fun completed(): ReceiveListener<IN, T> {
        return onCompleted?.invoke(this) ?: throw NullPointerException("you needless call this method ,because you're never used it in a receiveListener")
    }
    ...
    fun getQueryResult(compareData: T?): Boolean {
        ...
        try {
            ...
            if (queryData.isNotEmpty()) queryData.forEach {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other.getQueryData().size == map.size) {
            map.forEach { (k, v) ->
                val nameN = k.toInt()
                v.ac = "${v.type}$nameN${v.con}".toInt()
            }
        } else return false
        var endQueryAci = 0L
        queryData.forEach {
            endQueryAci += (map[it.name]?.ac ?: 0)
        }
        var endOtherAci = 0L
        other.getQueryData().forEach {
            endOtherAci += (map[it.name]?.ac ?: 0)
        }
        return endQueryAci == endOtherAci
    }

    private data class QueryEqualsInfo(val value: Any, val con: Int, val type: String, var ac: Int = 0)

    override fun hashCode(): Int {
        return super.hashCode() + queryData.hashCode()
    }
}
