...
import com.zj.preview.getDuration
import com.zj.preview.mod.ConversationFileInfo
import com.zj.preview.views.PreviewBaseLoadingView
...
class VideoView : FrameLayout, PlayerEvent {
    ...
    private var loadingView: PreviewBaseLoadingView? = null
    ...
    private fun init() {
        val root = View.inflate(context, R.layout.conversation_preview_video, this)
    ...
    override fun onLoading(path: String) {
        ...
        if (simpleVideoEventListener?.onLoading(path) == false) {
            if (!autoPlay) loadingView?.setMode(PreviewBaseLoadingView.DisplayMode.LOADING)
        }
    }
    ...
    override fun onPrepare(path: String, videoSize: Long) {
        ...
        if (simpleVideoEventListener?.onPrepare(path, videoSize) == false) {
            if (!autoPlay) loadingView?.setMode(PreviewBaseLoadingView.DisplayMode.DISMISS)
    ...
    override fun onError(e: Exception?) {
        if (simpleVideoEventListener?.onError(e) == false) {
            loadingView?.setMode(PreviewBaseLoadingView.DisplayMode.NO_DATA)
        }
    }
    ...
    fun playOrResume(data: ConversationFileInfo?) {
        data?.getPath()?.let {
            if (it == getPath()) {
                exoPlayer?.play()
            } else {
        ...
        } ?: throw NullPointerException("can not play with a null path!")
    }
