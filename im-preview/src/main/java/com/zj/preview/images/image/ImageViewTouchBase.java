...
import com.zj.preview.images.image.graphics.FastBitmapDrawable;
import com.zj.preview.images.image.utils.IDisposable;

/**
 * Base View to manage image zoom/scroll/pinch operations
 */
@SuppressWarnings("unused")
class ImageViewTouchBase extends AppCompatImageView implements IDisposable {
    ...
    protected Easing getEasing() {
        return ScaleEffect.CUBIC.INSTANCE.getEasing();
    }
    ...
    protected void scrollBy(float distanceX, float distanceY, final double durationMs) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                double x = getEasing().easeOut(currentMs, 0, dx, durationMs);
                double y = getEasing().easeOut(currentMs, 0, dy, durationMs);
    ...
    protected void zoomTo(float scale, float centerX, float centerY, final float durationMs) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                float newScale = (float) getEasing().easeInOut(currentMs, 0, deltaScale, durationMs);
